...
describe('Plus Home Test', ()=>{
    ...
    test('Render Div', ()=>{
        const courseID = '2381410CGV11';
        const courseName = 'Computer Graphics';

        const {getByTestId} = render(<PlusHome user = {user} courseID = {courseID} courseName = {courseName}/>);
        const d = getByTestId('++-div');
        expect(d).toBeTruthy();
    });
    ...
    test('onCreate', ()=>{
        ...
        const {getByTestId} = render(<PlusHome user = {user} courseID = {courseID} courseName = {courseName}/>);
        const btn = getByTestId('fab-oncreate');
        fireEvent.click(btn);
    });
    ...
    test('onFinish', ()=>{
        ...
        const {getByTestId} = render(<PlusHome user = {user} courseID = {courseID} courseName = {courseName}/>);
        const btn = getByTestId('fab-onfinish');
        fireEvent.click(btn);
    });
});
